...
**AI summary through Ollama.** With Ollama running and a chat model pulled, a
checkbox next to the search offers *AI summary (Ollama)*. It does not search
again: it takes the hits already on screen, sends them to the model in Ollama and
streams back a paragraph whose bracketed numbers jump to those very hits. So the
summary can never cite something you cannot look up, and the hit list underneath
stays exactly as it was — it complements the list rather than repeating it.

Everything about the box says what it is. An **AI** badge, then one sentence:
*Summary of the search results below — generated by an AI model in Ollama on
this machine*, then the model name. A footer spells out that this text does not come from your
archive but from a model in Ollama on this machine, that it rests solely on the
results below, and that a model this size errs more easily than Claude — when in
doubt, read the result itself.

Hits appear instantly; the model needs 15–60 seconds, so the two are deliberately
separate steps and the text streams in as it is written, cancellable at any time.
Without Ollama or without the model pulled, the checkbox is simply absent. Model
and number of sources live in the settings; `answer.py` holds the prompt and
streaming for both this and `rag_server.py`, so the rules cannot drift apart.

The point is not to beat Claude — via MCP Claude is faster and better. The point
is that the model runs in your own Ollama and nothing leaves the machine, which
for a mailbox archive is a reason of its own.
